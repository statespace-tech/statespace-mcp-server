...
if (command === "serve") {
    ...
    if (!targetArg)
        usage();
    const isRemote = targetArg.startsWith("http://") || targetArg.startsWith("https://");
    const target = isRemote ? targetArg : path.resolve(targetArg);
    startMcpServer(target, isRemote ? {} : collectProcessEnv()).catch((err) => {
    ...
    if (isRemote) {
        ...
        const filePath = await resolvePath(rootDir, pagePath).catch((e) => {
    ...
    if (!pagePath || runCommand.length === 0)
        usage();
    const isRemote = root.startsWith("http://") || root.startsWith("https://");
    if (isRemote) {
        const baseUrl = root.endsWith("/") ? root : root + "/";
        ...
        const result = await executeCommand(expandedCommand, path.dirname(filePath), mergedEnv).catch((e) => {
            process.stderr.write(`Error: ${e.message ?? String(e)}\n`);
            process.exit(1);
        });
        if (result.stdout)
