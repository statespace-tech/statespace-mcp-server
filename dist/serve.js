...
import { processComponentBlocks, hasComponentBlocks, } from "./runtime/components.js";
...
const BUNDLED_AGENTS_MD = readFileSync(path.join(__dirname, "prompts", "api", "AGENTS.md"), "utf8");
...
async function handleGet(req, res, rootDir, serverEnv, urlPath, queryEnv) {
    const envErr = validateEnvMap(queryEnv);
    if (envErr)
        return sendJsonError(res, 400, envErr);
    // Serve bundled AGENTS.md if the content directory doesn't have one
    const isAgentsRequest = urlPath === "/AGENTS.md" || urlPath === "AGENTS.md" ||
...
async function handlePost(res, rootDir, serverEnv, urlPath, bodyStr) {
    let request;
    try {
    ...
    if (!Array.isArray(request.command) ||
    ...
    try {
        ...
        return sendJsonError(res, 422, e.message);
    }
    const mergedEnv = mergeEvalEnv(serverEnv, requestEnv);
    const expandedCommand = expandCommandForExecution(request.command, frontmatter.specs, mergedEnv);
    if (!isValidToolCall(request.command, frontmatter.specs)) {
        return sendJsonError(res, 422, `Command not allowed: ${request.command.join(" ")}`);
    }
    const workingDir = path.dirname(filePath);
    try {
...
export function createServer(rootDir, serverEnv = {}) {
...
export function startServer(rootDir, options = {}) {
    ...
    server.listen(port, host, () => {
        process.stderr.write(`Statespace serving ${rootDir} at http://${host}:${port}\n`);
    });
}
