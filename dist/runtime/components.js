...
function findClosingFence(content) {
    let pos = 0;
...
function findNextEvalBlock(content, start) {
    let pos = start;
    while (pos < content.length) {
        const fenceIdx = content.indexOf("```", pos);
        if (fenceIdx === -1)
...
function parseEvalBlocks(content) {
    const blocks = [];
    let searchFrom = 0;
...
function executeEvalBlock(code, workingDir, userEnv) {
...
export async function processComponentBlocks(content, workingDir, userEnv) {
    ...
    const results = await Promise.all(blocks.map((block) => executeEvalBlock(block.code, workingDir, userEnv)));
...
export function hasComponentBlocks(content) {
