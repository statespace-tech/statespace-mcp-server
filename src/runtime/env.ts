...
function isValidEnvKey(key: string): boolean {
  if (key.length === 0 || key.length > MAX_ENV_VAR_KEY_BYTES) return false;
  return /^[A-Za-z_][A-Za-z0-9_]*$/.test(key);
}

/** Returns an error message string, or null if valid. */
export function validateEnvMap(env: Record<string, string>): string | null {
...
export function mergeEvalEnv(
  ...
  for (const [k, v] of Object.entries(untrusted)) {
    if (!isReservedEnvKey(k)) merged[k] = v;
  }
