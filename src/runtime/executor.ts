...
function validateCommandSecurity(command: string[]): void {
  ...
  if (cmd.includes("/") || cmd.includes("\\") || cmd.includes("..")) {
    throw new SecurityError(
      `Path separators not allowed in command name: ${cmd}`
    );
  }
      ...
      if (err.code === "ENOENT") {
        reject(
          new CommandNotFoundError(
            `Command '${cmd}' not found in PATH: ${sandboxPath}`
          )
        );
      } else if (err.code === "EACCES") {
        reject(
          new CommandNotFoundError(
            `Command '${cmd}' not executable in PATH: ${sandboxPath}`
          )
        );
      } else {
        reject(
          Object.assign(new Error(`Failed to execute ${cmd}: ${err.message}`), {
            statusCode: 500,
          })
        );
      }
